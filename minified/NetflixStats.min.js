let chartJSScript=document.createElement("script");chartJSScript.type="application/javascript";chartJSScript.src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/2.7.2/Chart.min.js";document.getElementsByTagName("head")[0].appendChild(chartJSScript);let flixInfo=window.netflix.reactContext.models.serverDefs.data;let userInfo=window.netflix.reactContext.models.userInfo.data;var shaktiHistoryURL=flixInfo.API_ROOT+"/shakti/"+flixInfo.BUILD_IDENTIFIER+"/viewingactivity?authURL="+userInfo.authURL+"&pg=";var flixStats={viewedItems:{},userDetails:{name:userInfo.name,guid:userInfo.guid,countryOfSignup:userInfo.countryOfSignup,currentCountry:userInfo.currentCountry,currentRegion:userInfo.currentRegion,membershipStatus:userInfo.membershipStatus,isInFreeTrial:userInfo.isInFreeTrial,isKids:userInfo.isKids}};function view(){var nameToWatched={};var dateToWatched={};for(var itemID in flixStats.viewedItems){var item=flixStats.viewedItems[itemID]
...
function main(){preLoad();console.log("Using API URL: "+shaktiHistoryURL);gatherWatchInfo(()=>{console.log("Finished gathering data");view();});}
main();