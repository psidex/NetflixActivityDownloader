let flixInfo=window.netflix.reactContext.models.serverDefs.data;let userInfo=window.netflix.reactContext.models.userInfo.data;var shaktiHistoryURL=flixInfo.API_ROOT+"/shakti/"+flixInfo.BUILD_IDENTIFIER+"/viewingactivity?authURL="+userInfo.authURL+"&pg=";var flixStats={viewedItems:{},userInfo:{name:userInfo.name,guid:userInfo.guid,countryOfSignup:userInfo.countryOfSignup,currentCountry:userInfo.currentCountry,currentRegion:userInfo.currentRegion,membershipStatus:userInfo.membershipStatus,isInFreeTrial:userInfo.isInFreeTrial,isKids:userInfo.isKids}};function view(){var nameToWatched={};var dateToWatched={};for(var itemID in flixStats.viewedItems){var item=flixStats.viewedItems[itemID]
...
function preLoad(){document.head.innerHTML+=`
    <style>
    h1 {
        color: #000;
    }
...
function loadExternal(){return new Promise((resolve,reject)=>{let chartJSScript=document.createElement("script");chartJSScript.type="application/javascript";chartJSScript.src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/2.7.2/Chart.min.js";chartJSScript.onload=()=>{console.log("Chart.js loaded")
resolve();};document.head.appendChild(chartJSScript);});}
function main(){console.log("Using API URL: "+shaktiHistoryURL);loadExternal().then(()=>{preLoad();}).then(()=>{gatherWatchInfo(()=>{console.log("Finished gathering data");view();});});}
main();