let script=document.createElement("script");script.type="application/javascript";script.src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/2.7.2/Chart.min.js";document.getElementsByTagName("head")[0].appendChild(script);document.head.innerHTML+=`
h1 {
    color: #000;
}
#NetflixStats {
...
var flixInfo=window.netflix.reactContext.models.serverDefs.data;var userInfo=window.netflix.reactContext.models.userInfo.data;var activityURL=flixInfo.API_ROOT+"/shakti/"+flixInfo.BUILD_IDENTIFIER+"/viewingactivity?"+"authURL="+userInfo.authURL+"&pgSize=100"+"&pg=";console.log("Using API URL: "+activityURL);var flix_stats={viewedItems:{},userDetails:{name:userInfo.name,guid:userInfo.guid,countryOfSignup:userInfo.countryOfSignup,currentCountry:userInfo.currentCountry,currentRegion:userInfo.currentRegion,membershipStatus:userInfo.membershipStatus,isInFreeTrial:userInfo.isInFreeTrial,isKids:userInfo.isKids}};var pageCount=0;function gatherWatchInfo(callback){console.log("Getting page "+pageCount);fetch(activityURL+pageCount).then((response)=>{return response.json();}).then((data)=>{if(data.viewedItems[0]===undefined){console.log("No viewed items in page\nFinished gathering pages");callback();}
else{for(var i=0;i<data.viewedItems.length;i++){var episodeData=data.viewedItems[i];var generalTitle="unknown";var generalType="series";var generalID;if(episodeData.seriesTitle){generalTitle=episodeData.seriesTitle
...
if(!flix_stats.viewedItems[generalID]){flix_stats.viewedItems[generalID]={}
flix_stats.viewedItems[generalID].title=generalTitle;flix_stats.viewedItems[generalID].type=generalType;if(generalType=="series"){flix_stats.viewedItems[generalID].watchCount=1;flix_stats.viewedItems[generalID].watchedEpisodes={[episodeData.movieID]:episodeObj};}else{flix_stats.viewedItems[generalID].dateWatched=episodeData.dateStr;flix_stats.viewedItems[generalID].duration=episodeData.duration;}}else{flix_stats.viewedItems[generalID].watchCount++;flix_stats.viewedItems[generalID].watchedEpisodes[episodeData.movieID]=episodeObj;}}
pageCount++;gatherWatchInfo(callback);}});}
gatherWatchInfo(()=>{console.log("Done");console.log(flix_stats);var nameToWatched={};var dateToWatched={};for(var itemID in flix_stats.viewedItems){var item=flix_stats.viewedItems[itemID]
